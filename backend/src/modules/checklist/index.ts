...
// Apply requireAuth and requireStudent to all checklist routes
checklistRouter.use('*', requireAuth, requireStudent);
...
checklistRouter.get('/:id/checklist', async (c) => {
...
checklistRouter.patch('/:id/checklist/:docId', async (c) => {
  const scholarshipId = c.req.param('id');
