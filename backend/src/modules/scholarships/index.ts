...
scholarshipsRouter.get('/', async (c) => {
  ...
  // Attach days_remaining and summary tags to each scholarship
...
adminScholarshipsRouter.get('/', async (c) => {
  ...
  const scholarships = (result.results || []).map((s: Record<string, unknown>) => ({
    ...s,
    days_remaining: calculateDaysRemaining(s.deadline as string),
    rules_count: Number(s.rules_count || 0),
    documents_count: Number(s.documents_count || 0),
  }));
